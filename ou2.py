...
def genera_tabella_grafica(risultati):
    """Genera una figura Matplotlib contenente la tabella riassuntiva formattata per OU 2D."""
    ...
    plt.title(
        r"TABELLA RIASSUNTIVA MODEL OU 2D ($v_x, v_y \in [-10, +10]$ m/s)",
        fontsize=13,
        weight="bold",
        pad=15,
    )
...
def analizza_ou2(nome_file, etichetta_rumore, rango_svd):
    ...
    # 3. PSEUDOINVERSE (Willems robusta vs SVD Troncata)
    ...
    target_grafici = [(900, 60), (900, 80), (900, 100)]
    dati_grafici = {}
    ...
    for (traj_idx, t_idx), dg in dati_grafici.items():
        plt.figure(figsize=(9, 5))
        plt.plot(dg["passato"][:, 0], dg["passato"][:, 1], "bo-", alpha=0.5, label="Passato Y")
        plt.plot(dg["futuro_y"][:, 0], dg["futuro_y"][:, 1], "go-", alpha=0.3, label="Futuro Y (rumoroso)")
        plt.plot(dg["futuro_x"][:, 0], dg["futuro_x"][:, 1], "k-", linewidth=2.5, label="Futuro X (vero)")
        plt.plot(dg["pred_willems"][:, 0], dg["pred_willems"][:, 1], "mX--", linewidth=1.5, label="Willems Std")
        plt.plot(dg["pred_svd"][:, 0], dg["pred_svd"][:, 1], "rD-.", linewidth=2, label=f"SVD (rango {rango_svd})")
        plt.title(rf"Predizione OU 2D ($v_x, v_y \in [-10, +10]$ m/s) - {etichetta_rumore}" + f"\n(Traiettoria {traj_idx}, t={t_idx})")
...
for percorso, etichetta, rango in database_list:
    try:
        res = analizza_ou2(percorso, etichetta, rango)
        risultati.append(res)
    except FileNotFoundError:
        print(f"File non trovato: {percorso} (verifica il nome esatto con ls data/)")

if risultati:
    print("\n" + "=" * 75)
    print("TABELLA RIASSUNTIVA MODEL OU 2D (vx,vy in [-10, +10] m/s)")
    print("=" * 75)
